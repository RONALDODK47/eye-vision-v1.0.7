...
function fixPlanoColumnBoundaries(columns: GenericColunaDef[], imgWidth: number): GenericColunaDef[] {
  ...
  for (let i = 0; i < dataCols.length - 1; i++) {
    ...
    a.end = Math.max(a.start + 4, split);
    b.start = Math.min(b.end - 4, split);
  }
...
function suggestPlanoColumnsByLayout(items: PosicionadoItem[], imgWidth: number): ColHit[] {
  ...
  const reduzidoX =
    reduzidoItems.length >= 4 ? medianPosicionadoX(reduzidoItems) : imgWidth * 0.06;
  const codeX =
    classItems.length >= 4
      ? medianPosicionadoX(classItems)
      : tipoX >= 0
        ? tipoX + Math.max(28, imgWidth * 0.06)
        : imgWidth * 0.22;
  const descX = medianPosicionadoX(textItems) || imgWidth * 0.42;
  const nivelX =
    grauItems.length >= 4
      ? medianPosicionadoX(grauItems.filter((it) => it.x > descX - 8))
      : imgWidth * 0.9;
  ...
  const hits: ColHit[] = [
    { id: 'codigoReduzido', x: reduzidoX, w: 36, y: headerY },
    { id: 'codigoClassificacao', x: codeX, w: 72, y: headerY },
    { id: 'descricao', x: descX, w: 120, y: headerY },
  ];
  if (tipoX >= 0) {
    hits.push({ id: 'tipo', x: tipoX, w: 22, y: headerY });
  }
  if (grauItems.length >= 4) {
    hits.push({ id: 'nivel', x: nivelX, w: 22, y: headerY });
  }
