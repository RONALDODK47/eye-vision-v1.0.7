...
import { canonicalColigadaDescricao, extractRegraEntityDescricao } from './extratoRegrasEntity';
import {
...
export function validateAiRegraSugestao(
  ...
  const coligadaHit = matchColigadaParaRegra(descricao, coligadas, extratoHistoricos);

  const historicoRef =
    extratoHistoricos.find((h) => normalizeExtratoMatchText(h).length >= 6) || descricao;
  ...
  if (coligadaHit) {
    ...
    descricao = canonicalColigadaDescricao(coligadaHit);
    return {
      descricao,
      nature,
      contaContrapartida: coligConta,
      motivo: sug.motivo || 'Coligada — não fornecedor/cliente',
    };
  }
  ...
  // ——— FORNECEDOR / CLIENTE GENÉRICO: conta geral + padrão operacional ———
  ...
  if (
    ...
    (isContaImpostoOuTributo(conta.name) ||
      (isContaOperacionalEspecial(conta.name) && !isContaGeralFornecedorOuCliente(conta.name)))
  ) {
    ...
    return {
      ...
      motivo: sug.motivo || 'PIX/TED genérico — não imposto/tarifa',
    };
  }

  if (sentidoOk && !coligadaHit && !(generico && nominal)) {
    if (
  ...
  if (generico || nominal) {
    ...
    const geral = pickFallbackContaPorNatureza(nature, plano);
    if (!geral) return null;
    const melhor = pickBestPlanoContasParaHistorico(historicoRef, nature, plano, 1)[0];
    const melhorNominal = melhor
      ? plano.find(
          (p) =>
            (sanitizeCodigoReduzido(p.codigoReduzido) || sanitizeCodigoReduzido(p.code)) ===
            melhor.reduzido,
    ...
    if (
      melhor &&
      melhor.score >= 40 &&
      melhor.reduzido !== geral &&
      !(generico && melhorNominal && isContaNominalEmpresa(melhorNominal.name))
    ) {
      return {
        descricao: extractRegraEntityDescricao(historicoRef, nature, coligadas) || descricao,
        nature,
        contaContrapartida: melhor.reduzido,
        motivo: sug.motivo || `Match plano: ${melhor.name}`,
      };
    }
    descricao = extractPadraoOperacionalAgrupado(historicoRef, nature);
    ...
    return {
      ...
      nature,
      contaContrapartida: geral,
      motivo: sug.motivo || 'Agrupado em conta geral (precisão)',
    };
  }
...
export function validateAiRegrasLote(
  ...
  for (const sug of sugestoes) {
    const v = validateAiRegraSugestao(sug, plano, coligadas, anexosTexto, extratoHistoricos);
    if (!v) continue;
    ...
    seen.add(key);
    out.push({ ...v, descricao: entity || v.descricao });
  }
  return out;
}
