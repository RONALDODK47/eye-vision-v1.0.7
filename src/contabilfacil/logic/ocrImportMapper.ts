...
function inferPlanoTipoFromFields(
  ...
  const hint = tipoHint?.trim().toUpperCase();
  ...
  if (nivel != null && nivel <= 4 && dots <= 3) return 'S';
  if (dots >= 4) return 'A';
  if (dots <= 1) return 'S';
  return undefined;
}
...
function inferPlanoFromOcrRow(row: GenericOcrRow): {
  ...
  const allText = [row.codigoReduzido, code, name, row.tipo, row.nivel, row._linhaOcr]
    .filter(Boolean)
    .join(' ')
    .replace(/\s+/g, ' ')
    .trim();
  ...
  const dominioTabular = linhaFundida
    ? allText.match(/^(\d{1,7})\s+([SA])\s+(\d[\d.]{0,18})\s+(.+?)(?:\s+([1-6]))?\s*$/i)
    : null;
  const dominioRelatorioSa = linhaFundida
    ? allText.match(/^(?:1\s+)?(\d{1,4})\s+([SA])\s+(\d+(?:\.\d+)*)\s+(.+?)\s+([1-6])\s*$/i)
    : null;
  const dominioRelatorioAnalitica = linhaFundida
    ? allText.match(/^(?:1\s+)?(\d{1,4})\s+(\d+(?:\.\d+){1,}(?:\.\d{2,5})?)\s+(.+?)\s+([1-6])\s*$/i)
    : null;
  const dominioComTipo = linhaFundida
    ? allText.match(/^(\d{5,7})\s+(\d[\d.\s]{2,18})\s+(.+?)\s+([SA])\s*$/i)
    : null;
  const dominioSemTipo = linhaFundida
    ? allText.match(/^(\d{5,7})\s+(\d[\d.\s]{2,18})\s+(.+)$/i)
    : null;
  if (dominioTabular) {
  ...
  code = code.replace(/\s+/g, '').replace(/[^\d.]/g, '') || code.trim();
  ...
  const tipo =
    inferPlanoTipoFromFields(code, nivel, tipoHint) ??
