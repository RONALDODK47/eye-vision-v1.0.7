...
import { ChevronLeft, ChevronRight, FileSpreadsheet, Filter, HelpCircle, Plus, Trash2, X } from 'lucide-react';
...
export function GenericLeitorTable({
  ...
  const handleApplyExclusionPermanently = () => {
    ...
    if (
      window.confirm(
        `Deseja excluir definitivamente as ${excludedRowsCount} linhas filtradas? Esta ação não pode ser desfeita.`,
      )
    ) {
        ...
        <div className="px-4 py-2 bg-white/60 border-b border-brand-border flex flex-wrap items-center gap-3 shrink-0">
          ...
          {excludedRowsCount > 0 && (
            <button
              ...
              className="flex items-center gap-1 px-2 py-1 bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-200 text-[10px] font-semibold cursor-pointer"
            >
              <Trash2 className="w-3 h-3" />
              Excluir ({excludedRowsCount})
            </button>
          )}
        </div>
      )}
          ...
          >
            {showCrops ? (
              <div className="flex flex-col gap-1.5 min-w-0">
                <h3 className="text-[10px] font-bold text-brand-text uppercase tracking-wider px-1">
            ...
            <div className="flex flex-col gap-1.5 min-w-0">
              <h3 className="text-[10px] font-bold text-emerald-700 uppercase tracking-wider px-1">
